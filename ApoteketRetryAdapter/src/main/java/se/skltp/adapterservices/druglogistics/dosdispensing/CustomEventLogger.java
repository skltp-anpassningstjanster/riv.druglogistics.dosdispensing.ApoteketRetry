...
public class CustomEventLogger implements EventLogger, MuleContextAware {
	...
	private static final Logger log = LoggerFactory.getLogger(CustomEventLogger.class);
	...
	private static final String LOG_EVENT_WARN = "logEvent-warning"; // ML FIXME Added
	...
	public void logErrorEvent(LogLevelType logLevel, Throwable error,
		...
		// ML FIXME. Added the logMEssage in the call...
		logErrorEvent(logLevel, elm.getLogMessage(), error, elm.getMuleMessage(),
	...
	/**
	 * ML FIXME. Added in CustomEventLogger to allow logMessage have reference over error-message as the log-message...
	 * 
	 * @param logLevel
	 * @param error
	 * @param message
	 * @param integrationScenario
	 * @param contractId
	 * @param businessContextId
	 * @param extraInfo
	 */
	protected void logErrorEvent(LogLevelType logLevel, String logMessage, Throwable error,
			MuleMessage message, String integrationScenario, String contractId,
			Map<String, String> businessContextId, Map<String, String> extraInfo) {

		if (logMessage == null) logMessage = error.toString();
		
		LogEvent logEvent = createLogEntry(logLevel, message, logMessage,
				integrationScenario, contractId, businessContextId, extraInfo,
				message.getPayload(), error);

		// ML FIXME Added to allow to warnings to be logged as well...
		String logEventName = LOG_EVENT_ERROR;
